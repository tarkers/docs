...
想知道更多 Horizon 的資訊，請查閱[完整的 Horizon 文件](/docs/{{version}}/horizon)
...
關於升級套件來使用服務提供者和 Facade 發掘的更多相關資訊，請查閱[套件開發](/docs/{{version}}/packages)的完整文件。
...
當然，這只是一個 API 資源的最基本範例。Laravel 也提供各種方法來協助你建構 Resource 和 Resource 的集合。想知道更多的資訊，請查閱[API 資源](/docs/{{version}}/eloquent-resources)的完整文件。
